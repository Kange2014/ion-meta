...
fn_order_classify_results <- gsub(".tsv",".xls",snakemake@input[[3]])
write.table(order_classify_results,file=fn_order_classify_results,sep="\t",row.names=FALSE)
...
```{r download_raw_data}
    ...
    embed_files = function(path, name = with_ext(gsub("_se.report.xls","",basename(path[1])), 'RawClassifyResults.zip'), ...) {
    ...
    embed_files(c(fn_order_classify_results,snakemake@input[[4]]))
```
